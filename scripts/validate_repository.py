...
def freshness_shell_definitions_are_safe(text: str) -> bool:
    ...
    if (
        ...
        return False
    segments = shell_command_segments(text.replace(FRESHNESS_CHECKER_STATUS_GUARD, ""))
    if segments is None:
        return False
    if any(
...
def freshness_checker_result_controls_reconciliation(text: str) -> bool:
    ...
    if any(
        command not in FRESHNESS_ALLOWED_PRINTF_COMMANDS for command in printf_commands
    ):
        return False
...
def freshness_command_order_is_valid(text: str) -> bool:
    ...
    phase_text = text.replace(FRESHNESS_CHECKER_STATUS_GUARD, "")
    audit_positions: list[int] = []
    api_positions: list[int] = []
    mutation_positions: list[int] = []
    lexer = shlex.shlex(phase_text, posix=True, punctuation_chars=True)
    lexer.whitespace_split = True
    lexer.commenters = "#"
    try:
        ...
        return False
    segments = shell_command_segments(phase_text)
    if segments is None:
        return False
...
def validate_code_scanning_gate_contract(repository_root: Path) -> list[str]:
    ...
    for path in (
        ...
        if (
            ...
            problems.append(
                f"{relative}: must use one unconditional trusted code-scanning gate contract"
            )
            continue
        ...
        if (
            ...
            or '--pull-request "$PR_NUMBER"' not in run_text
            or '--base-sha "$PR_BASE_SHA"' not in run_text
            or '--head-sha "$PR_HEAD_SHA"' not in run_text
            or '--ref "$MERGE_GROUP_REF"' not in run_text
            or '--sha "$MERGE_GROUP_SHA"' not in run_text
            or not all(category in run_text for category in expected_categories)
        ...
        expected.update(
            {
                ...
                "repo-scaffold/scripts/check_code_scanning_alerts.py",
