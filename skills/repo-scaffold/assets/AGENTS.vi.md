...
hoặc `--template dependency-update`. Khi title Conventional Commit bắt đầu bằng
`feat`, `fix` hoặc `docs`, lần lượt dùng `feature`, `bugfix` hoặc
`documentation`. Gate `pr-template` bắt buộc các mapping này và từ chối
override. Giữ đúng một marker `<!-- repo-scaffold:pr-template=<id> -->` phù hợp, mọi heading bắt
